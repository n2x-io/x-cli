...
func (a *API) Delete(ns, name string) error {
	for {
		...
		if err := a.clientset.CoreV1().Services(ns).Delete(ctx, name, deleteOpts); err != nil {
			return errors.Wrapf(err, "[%v] function a.clientset.CoreV1().Services().Delete()", errors.Trace())
		}
